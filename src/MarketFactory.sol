...
import {ERC6909Minimal} from "./tokens/ERC6909Minimal.sol";
import {IPredictionAMM} from "./interface/IPredictionAMM.sol";

/// @title MarketFactory V1
...
/// @dev Trading/LP happens on a separate AMM (e.g. PredictionAMM).
contract MarketFactory is ERC6909Minimal {
    /*//////////////////////////////////////////////////////////////
                                 ERRORS
    //////////////////////////////////////////////////////////////*/
    ...
    struct Market {
        address resolver; // who can resolve (20 bytes) --> Oracle address
        bool resolved; // outcome set? (1 byte) --> The answer is out or not
        bool outcome; // YES wins if true (1 byte) --> The answer is YES or NO
        bool canClose; // resolver can early-close (1 byte) --> Can the oracle close early
        uint64 close; // resolve allowed after (8 bytes) -- slot 1: 31 bytes --> Timestamp after which oracle can resolve
        address collateral; // collateral token (address(0) = ETH) -- slot 2 --> Collateral token address
        uint256 collateralLocked; // collateral locked for market -- slot 3 --> Amount of collateral locked in the market
    }
    ...
    function name(uint256 id) public pure returns (string memory) {
        return string(abi.encodePacked("MarketFactory-", _toString(id)));
    }
    ...
    function tokenURI(uint256 id) public view returns (string memory) {
        ...
        if (m.resolver != address(0)) { // This mean market exists
        ...
        return "data:application/json;utf8,{\"name\":\"MarketFactory NO Share\",\"description\":\"Prediction market NO share\"}";
    }
    ...
    function transferFrom(address sender, address receiver, uint256 id, uint256 amount) // 
        ...
        returns (bool)
    {
        // PredictionAMM pulls from address(this) - skip SLOAD for isOperator/allowance
        if (
            msg.sender != sender // Not self transfer
        && 
        !(msg.sender == address(PredictionAMM) // 
        && 
        sender == address(this))) 
    ...
    /// @param s Signature s
    function permit(
        address token, // The ERC20 token address like USDC, DAI etc
        address owner, // Token Owner address who signed the permit
        uint256 value, // Amount to approve to this contract
    ...
    function createMarketAndSeed(
        string calldata description, // Question to be asked in the market
        address resolver, // Oracle address who can resolve the market
    ...
    function _createMarket(
        ...
        marketId = getMarketId(description, resolver, collateral); // MarketId or YES id is generated from description, resolver and collateral
        if (markets[marketId].resolver != address(0)) revert MarketExists(); // If the market already exists then revert

        noId = getNoId(marketId); // NO id is generated from YES id
    ...
    function closeMarket(uint256 marketId) public nonReentrant { // This function is used to change the closing time, not to resolve the market
    ...
    function poolKey(uint256 marketId, uint256 feeOrHook)
        public
        view
        returns (IPredictionAMM.PoolKey memory key)
    {
    ...
    function _splitAndAddLiquidity(
        uint256 marketId, // The market to add liquidity for
        uint256 collateralIn, // Amount of collateral to split
        uint256 feeOrHook, // Pool fee tier (bps) or hook address
        uint256 amount0Min, // Minimum amount of token0 to add (slippage protection)
        uint256 amount1Min, // Minimum amount of token1 to add (slippage protection)
        uint256 minLiquidity, // Minimum LP tokens to receive
        address to, // Recipient of LP tokens
        uint256 deadline // Timestamp after which the tx reverts (0 = current block)
        ...
        uint256 noId = getNoId(marketId);
        IPredictionAMM.PoolKey memory key = _poolKey(marketId, noId, feeOrHook);
        PredictionAMM.deposit(address(this), key.id0, shares);
        PredictionAMM.deposit(address(this), key.id1, shares);

        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        (,, liquidity) = PredictionAMM.addLiquidity(key, shares, shares, amount0Min, amount1Min, to, dl);
        if (liquidity < minLiquidity) revert InsufficientOutput();

        PredictionAMM.recoverTransientBalance(address(this), key.id0, msg.sender);
        PredictionAMM.recoverTransientBalance(address(this), key.id1, msg.sender);
    }
    ...
    /// @dev User must approve MarketFactory on PredictionAMM to pull LP tokens (via PredictionAMM.setOperator or approve).
    ...
    function removeLiquidityToCollateral(
        ...
        PredictionAMM.transferFrom(msg.sender, address(this), poolId, liquidity);

        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        (uint256 a0, uint256 a1) =
            PredictionAMM.removeLiquidity(key, liquidity, amount0Min, amount1Min, address(this), dl);
    ...
    function buyYes(
        ...
        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        PredictionAMM.deposit(address(this), noId, shares);
        uint256 yesFromSwap =
            PredictionAMM.swapExactIn(key, shares, minSwapOut, zeroForOne, address(this), dl);
        PredictionAMM.recoverTransientBalance(address(this), noId, address(this));
    ...
    function buyNo(
        ...
        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        PredictionAMM.deposit(address(this), marketId, shares);
        uint256 noFromSwap =
            PredictionAMM.swapExactIn(key, shares, minSwapOut, zeroForOne, address(this), dl);
        PredictionAMM.recoverTransientBalance(address(this), marketId, address(this));
    ...
    function sellYes(
        ...
        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        PredictionAMM.deposit(address(this), marketId, yesToSwap);
        uint256 noFromSwap =
            PredictionAMM.swapExactIn(key, yesToSwap, minSwapOut, zeroForOne, address(this), dl);
        PredictionAMM.recoverTransientBalance(address(this), marketId, address(this));
    ...
    function sellNo(
        ...
        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        PredictionAMM.deposit(address(this), noId, noToSwap);
        uint256 yesFromSwap =
            PredictionAMM.swapExactIn(key, noToSwap, minSwapOut, zeroForOne, address(this), dl);
        PredictionAMM.recoverTransientBalance(address(this), noId, address(this));
    ...
    function sellYesForExactCollateral(
        ...
        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        PredictionAMM.deposit(address(this), marketId, maxYesIn);
        uint256 yesSwapped =
            PredictionAMM.swapExactOut(key, merged, maxSwapIn, zeroForOne, address(this), dl);
        PredictionAMM.recoverTransientBalance(address(this), marketId, address(this));
    ...
    function sellNoForExactCollateral(
        ...
        uint256 dl = deadline == 0 ? block.timestamp : deadline;
        PredictionAMM.deposit(address(this), noId, maxNoIn);
        uint256 noSwapped = PredictionAMM.swapExactOut(key, merged, maxSwapIn, zeroForOne, address(this), dl);
        PredictionAMM.recoverTransientBalance(address(this), noId, address(this));
    ...
    {
        IPredictionAMM.PoolKey memory key = poolKey(marketId, feeOrHook);
        uint256 pid =
            uint256(keccak256(abi.encode(key.id0, key.id1, key.token0, key.token1, key.feeOrHook)));
        (uint112 r0, uint112 r1,,,,,) = PredictionAMM.pools(pid);
